...
const forLoopTwoToThe = (arr) => {
    let squares = arr.map(n => Math.pow(2, n));
...
const charCode = (arr) => {
    let codes = arr.map(n => n.charCodeAt());
    return codes;
};
...
const evenOdd = (arr) => {
    const oddEvenStrings = arr.map(n => {
        if (typeof n !== 'number') {
            return 'N/A';
        }
        return n % 2 === 0 ? 'even' : 'odd';
    })
    console.log(oddEvenStrings);
...
const extractStats = (arr) => {
    const extractedStats = arr.map(stats => ({
