...
const getCourseKeys = (obj) => {
  const courseInfoKeys = Object.keys(obj);
  return courseInfoKeys;
}
...
const getHouses = (arr) => {
  let houses = [];
  arr.forEach(houseName => houses.push(houseName.house));
  console.log(houses);
...
const houseSurvivors = (arr) => {
  ...
  Object.values(arr).forEach(person => {
    let houseMembers = 1;
    if(person.spouse && !deceasedSpouses.includes(person.spouse)) houseMembers++;
    person.children.forEach(() => houseMembers++);
    survivors.push({
      house: person.house,
      members: houseMembers,
